...
/**
 * Write a description of class ThreadedClassficationWeka here.
 *
 * @author (your name)
 * @version (a version number or a date)
 */
public class ThreadedClassficationWeka implements Runnable
{
    ...
    public void run() {
        try {
            ...
            // Vector to house threads
            //java.util.List<ClassifyThread> threads = new java.util.Vector<ClassifyThread>();
            ...
            for (int i = 0; i < NUMTHREADS; i++) {
                ...
                if (i == NUMTHREADS-1) {
                    sublist = animalsToClassify.subList(2*i, animalsToClassify.size());
                }
                else {
                    sublist = animalsToClassify.subList(2*i, 2*i+2);
                }
                ClassifyThread ct = new ClassifyThread(i+1, sublist);
                ct.run();
                //threads.add(ct);
            }
            ...
            // Run Threads
            // for (int i = 0; i < NUMTHREADS; i++) {
            // ClassifyThread runMe = threads.get(i);
            // runMe.start();
            // }
    ...
    public static void main(String[] args) {
        javax.swing.SwingUtilities.invokeLater(new ThreadedClassficationWeka());
    }
}
...
class ClassifyThread extends Thread
{
    ...
    private java.util.List<double[]> myAnimals;

    /**
     * Constructor for objects of class ClassifyThread
     */
    public ClassifyThread(int num, java.util.List<double[]> sublist)
    {
        threadNum = num;
        myAnimals = sublist;
    }
    ...
    public void run() {
        try {
            for (int i = 0; i < myAnimals.size(); i++) {
                ...
                Instance myAnimal = new DenseInstance(1.0, myAnimals.get(i));
                myAnimal.setDataset(ThreadedClassficationWeka.data);
                double result = ThreadedClassficationWeka.tree.classifyInstance(myAnimal);
                System.out.print("From thread " + getThreadNum() + " ANIMAL TYPE = ");
                System.out.println("ANIMAL TYPE = " + ThreadedClassficationWeka.data.classAttribute().value((int) result));
